...
import { cv } from '@/lib/supabase/supabase.types';
...
const DashboardSetup = ({ user }: DashboardSetupProps) => {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof CreateWorkspaceFormSchema>> = async (value) => {
    ...
    try {
      ...
      const { data, error: createError } = await createWorkspace(newCv);
      if (createError) {
        throw new Error();
      }
      dispatch({
        type: 'ADD_CV',
        payload: { ...newCv, folders: [] },
      });

      toast({
        title: 'CV Created',
        description: `${newCv.title} has been created successfully.`,
      });

      router.replace(`/dashboard/${newCv.id}`);
    } catch (error) {
      console.log(error, 'Error');
      toast({
        variant: 'destructive',
        title: 'Could not create your CV',
        description: "Oops! Something went wrong, and we couldn't create your CV. Try again or come back later.",
      });
    } finally {
      reset();
    }
  ...
  "
    >
      <CardHeader>
        <CardTitle>Create A CV</CardTitle>
        <CardDescription>
